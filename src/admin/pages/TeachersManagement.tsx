...
const TeachersManagement: React.FC<TeachersManagementProps> = ({ users, onRefresh }) => {
  ...
  const handleToggleStatus = async (userId: number, currentStatus: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`تم ${currentStatus ? 'إلغاء تفعيل' : 'تفعيل'} المدرس بنجاح`);
        onRefresh();
      } else {
        toast.error('حدث خطأ في تحديث حالة المدرس');
      }
    } catch (error) {
      toast.error('حدث خطأ في تحديث حالة المدرس');
    } finally {
    ...
    if (window.confirm('هل أنت متأكد من حذف هذا المدرس؟')) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          toast.error('حدث خطأ في حذف المدرس');
        }
      } catch (error) {
        toast.error('حدث خطأ في حذف المدرس');
      } finally {
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">إدارة المدرسين</h1>
        <p className="text-gray-600">إدارة وتتبع جميع المدرسين في المنصة</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-600">المدرسين النشطين</p>
          ...
          >
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-gray-800">تفاصيل المدرس</h3>
                <button
